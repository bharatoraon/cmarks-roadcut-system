    ...
    return (
        <div className="glass-panel" style={{
            ...
            <div style={{ flex: 1, overflowY: 'auto' }}>
                {!selectedRoad ? (
                    <div className="animate-fade-in" style={{ textAlign: 'center', padding: '40px 0', color: 'var(--text-secondary)' }}>
                        <Info size={48} style={{ marginBottom: '16px', opacity: 0.5 }} />
                        <p>Select a road on the map to view details or apply for a road cut permission.</p>
                    </div>
                ) : (
                    <div className="animate-fade-in">
                        <h2 style={{ fontSize: '1.1rem', marginBottom: '16px', color: 'var(--accent-primary)' }}>Selected Asset</h2>
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                            ...
                            <button
                                className="btn-primary"
                                style={{ background: '#475569', border: 'none', color: 'white', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px' }}
                            ...
                            <button
                                className="btn-primary"
                                style={{ gridColumn: '1 / -1', background: '#0f766e', color: 'white', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px', border: 'none' }}
                                onClick={onNavigate}
                            >
                                <Navigation size={18} />
...
function InfoRow({ label, value }) {
    return (
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'start', gap: '12px', padding: '8px 0', borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
            <span style={{ color: 'var(--text-secondary)', fontSize: '0.9rem', flexShrink: 0 }}>{label}</span>
            <span style={{ fontWeight: 500, textAlign: 'right', wordBreak: 'break-word', fontSize: '0.95rem', lineHeight: '1.4' }}>{value}</span>
        </div>
    );
}
